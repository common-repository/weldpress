...
add_filter( 'WELD_PRESS_spam', 'WELD_PRESS_akismet' );

function WELD_PRESS_akismet( $spam ) {
	if ( $spam ) {
		return $spam;
	}

	if ( ! WELD_PRESS_akismet_is_available() ) {
		return false;
	}

	if ( ! $params = WELD_PRESS_akismet_submitted_params() ) {
		return false;
	}
	...
	return WELD_PRESS_akismet_comment_check( $c );
}

function WELD_PRESS_akismet_is_available() {
...
function WELD_PRESS_akismet_submitted_params() {
	$params = array(
		'author' => '',
		'author_email' => '',
		'author_url' => '',
	...
	foreach ( (array) $_POST as $key => $val ) {
		if ( '_WELDPRESS' == substr( $key, 0, 6 ) || '_wpnonce' == $key ) {
			continue;
		}

		if ( is_array( $val ) ) {
			$val = implode( ', ', WELD_PRESS_array_flatten( $val ) );
		}
		...
		if ( $tags = WELD_PRESS_scan_form_tags( array( 'name' => $key ) ) ) {
			$tag = $tags[0];
...
function WELD_PRESS_akismet_comment_check( $comment ) {
	global $akismet_api_host, $akismet_api_port;

	$spam = false;
	$query_string = WELD_PRESS_build_query( $comment );
	...
	return apply_filters( 'WELD_PRESS_akismet_comment_check', $spam, $comment );
}
