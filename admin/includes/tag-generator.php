...
class WELD_PRESS_TagGenerator {
	...
	public function print_buttons() {
		...
		foreach ( (array) $this->panels as $panel ) {
			echo sprintf(
				...
				esc_attr( sprintf(
					__( 'Form-tag Generator: %s', 'weld-press' ),
					$panel['title'] ) ),
	...
	public function print_panels( WELD_PRESS_Application $application_form ) {
		foreach ( (array) $this->panels as $id => $panel ) {
			...
			if ( is_callable( $callback ) ) {
				...
				call_user_func( $callback, $application_form, $options );
