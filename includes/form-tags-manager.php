...
function WELD_PRESS_add_form_tag( $tag, $func, $has_name = false ) {
	$manager = WELD_PRESS_FormTagsManager::get_instance();

	return $manager->add( $tag, $func, $has_name );
}

function WELD_PRESS_remove_form_tag( $tag ) {
	$manager = WELD_PRESS_FormTagsManager::get_instance();

	return $manager->remove( $tag );
}

function WELD_PRESS_replace_all_form_tags( $content ) {
	$manager = WELD_PRESS_FormTagsManager::get_instance();

	return $manager->replace_all( $content );
}

function WELD_PRESS_scan_form_tags( $cond = null ) {
	$contact_form = WELD_PRESS_Application::get_current();
...
class WELD_PRESS_FormTagsManager {
	...
	private function scan_callback( $m, $replace = false ) {
		...
		if ( is_array( $attr ) ) {
			if ( is_array( $attr['options'] ) ) {
				if ( $this->tags[$tag]['has_name'] && ! empty( $attr['options'] ) ) {
					...
					if ( ! WELD_PRESS_is_name( $scanned_tag['name'] ) ) {
			...
			if ( WELD_PRESS_USE_PIPE ) {
				$pipes = new WELD_PRESS_Pipes( $scanned_tag['raw_values'] );
		...
		$scanned_tag = apply_filters( 'WELD_PRESS_form_tag', $scanned_tag, $replace );
	...
	private function parse_atts( $text ) {
		...
		if ( preg_match( $pattern, $text, $match ) ) {
			...
			if ( ! empty( $match[2] ) ) {
				...
				$atts['values'] = WELD_PRESS_strip_quote_deep( $matched_values[0] );
			}
		} else {
			$atts = $text;
		}
