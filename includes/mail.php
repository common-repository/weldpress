...
class WELD_PRESS_Mail {
	...
	private function htmlize( $body ) {
		$header = apply_filters( 'WELD_PRESS_mail_html_header',
			'<!doctype html>
...
', $this );
		...
		$footer = apply_filters( 'WELD_PRESS_mail_html_footer',
			'</body>
</html>', $this );
	...
	private function compose( $send = true ) {
		...
		$components = apply_filters( 'WELD_PRESS_mail_components',
			$components, WELD_PRESS_get_current_contact_form(), $this );

		if ( ! $send ) {
			return $components;
		}

		$subject = WELD_PRESS_strip_newline( $components['subject'] );
		$sender = WELD_PRESS_strip_newline( $components['sender'] );
		$recipient = WELD_PRESS_strip_newline( $components['recipient'] );
		...
		if ( $this->use_html ) {
			...
			$headers .= "X-WELDPRESS-Content-Type: text/html\n";
		} else {
			$headers .= "X-WELDPRESS-Content-Type: text/plain\n";
		}
	...
	public function replace_tags( $content, $args = '' ) {
		...
		return WELD_PRESS_mail_replace_tags( $content, $args );
	}
...
function WELD_PRESS_mail_replace_tags( $content, $args = '' ) {
	$args = wp_parse_args( $args, array(
		'html' => false,
		'exclude_blank' => false ) );
	...
	if ( is_array( $content ) ) {
		foreach ( $content as $key => $value ) {
			$content[$key] = WELD_PRESS_mail_replace_tags( $value, $args );
		}
	...
	foreach ( $content as $num => $line ) {
		$line = new WELD_PRESS_MailTaggedText( $line, $args );
...
add_action( 'phpmailer_init', 'WELD_PRESS_phpmailer_init' );

function WELD_PRESS_phpmailer_init( $phpmailer ) {
	$WELD_PRESS_content_type = false;

	foreach ( (array) $phpmailer->getCustomHeaders() as $custom_header ) {
		if ( 'X-WELDPRESS-Content-Type' == $custom_header[0] ) {
			$WELD_PRESS_content_type = trim( $custom_header[1] );
			break;
		}
	}

	if ( 'text/html' == $WELD_PRESS_content_type ) {
		$phpmailer->msgHTML( $phpmailer->Body );
	}
}

class WELD_PRESS_MailTaggedText {
	...
	private function replace_tags_callback( $matches, $html = false ) {
		...
		if ( null !== $submitted ) {
			...
			$replaced = WELD_PRESS_flat_join( $replaced );
			...
			$replaced = apply_filters( 'WELD_PRESS_mail_tag_replaced',
		...
		$special = apply_filters( 'WELD_PRESS_special_mail_tags', '', $tagname, $html );
...
add_filter( 'WELD_PRESS_special_mail_tags', 'WELD_PRESS_special_mail_tag', 10, 3 );

function WELD_PRESS_special_mail_tag( $output, $name, $html ) {
	$name = preg_replace( '/^WELDPRESS\./', '_', $name ); // for back-compat
	...
	if ( '_post_' == substr( $name, 0, 6 ) ) {
		...
		if ( $unit_tag
		&& preg_match( '/^WELDPRESS-f(\d+)-p(\d+)-o(\d+)$/', $unit_tag, $matches ) ) {
