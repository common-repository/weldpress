...
class WELD_PRESS_Application {

	const post_type = 'weldp_press_solution';
	...
	public static function register_post_type() {
		register_post_type( self::post_type, array(
			'labels' => array(
				'name' => __( 'WeldPress Solution', 'weld-press' ),
				'singular_name' => __( 'Contact Form', 'weld-press' ) ),
			'rewrite' => false,
			'query_var' => false ) );
	}
	...
	public static function get_template( $args = '' ) {
		...
		if ( $locale ) {
			$mo_orig = $l10n['weld-press'];
			WELD_PRESS_load_textdomain( $locale );
		}

		self::$current = $contact_form = new self;
		$contact_form->title =
			( $title ? $title : __( 'Untitled', 'weld-press' ) );
		$contact_form->locale = ( $locale ? $locale : get_locale() );

		$properties = $contact_form->get_properties();

		foreach ( $properties as $key => $value ) {
			$properties[$key] = WELD_PRESS_ApplicationTemplate::get_default( $key );
		}

		$contact_form->properties = $properties;

		$contact_form = apply_filters( 'WELD_PRESS_application_default_pack',
			$contact_form, $args );

		if ( isset( $mo_orig ) ) {
			$l10n['weld-press'] = $mo_orig;
		}
	...
	private static function get_unit_tag( $id = 0 ) {
		...
		if ( in_the_loop() ) {
			$unit_tag = sprintf( 'WELDPRESS-f%1$d-p%2$d-o%3$d',
				absint( $id ), get_the_ID(), $global_count );
		} else {
			$unit_tag = sprintf( 'WELDPRESS-f%1$d-o%2$d',
	...
	private function __construct( $post = null ) {
		...
		do_action( 'WELD_PRESS_application', $this );
	}

	public function __get( $name ) {
		$message = __( '<code>%1$s</code> property of a <code>WELD_PRESS_Application</code> object is <strong>no longer accessible</strong>. Use <code>%2$s</code> method instead.', 'weld-press' );
	...
	public function get_properties() {
		...
		$properties = (array) apply_filters( 'WELD_PRESS_application_properties',
			$properties, $this );
	...
	public function is_posted() {
		if ( ! WELD_PRESS_Submission::get_instance() ) {
			return false;
		}

		if ( empty( $_POST['_WELD_PRESS_unit_tag'] ) ) {
			return false;
		}

		return $this->unit_tag == $_POST['_WELD_PRESS_unit_tag'];
	}
	...
	public function form_html( $args = '' ) {
		...
		if ( 'raw_form' == $args['output'] ) {
			return '<pre class="WELDPRESS-raw-form"><code>'
		...
		$html = sprintf( '<div %s>', WELD_PRESS_format_atts( array(
			'role' => 'form',
			'class' => 'WELDPRESS',
			'id' => $this->unit_tag,
			( get_option( 'html_type' ) == 'text/html' ) ? 'lang' : 'xml:lang'
				=> $lang_tag,
			'dir' => WELD_PRESS_is_rtl( $this->locale ) ? 'rtl' : 'ltr' ) ) ) . "\n";

		$html .= $this->screen_reader_response() . "\n";

		$url = WELD_PRESS_get_request_uri();
		...
		$url = apply_filters( 'WELD_PRESS_form_action_url', $url );

		$id_attr = apply_filters( 'WELD_PRESS_form_id_attr',
			preg_replace( '/[^A-Za-z0-9:._-]/', '', $args['html_id'] ) );

		$name_attr = apply_filters( 'WELD_PRESS_form_name_attr',
			preg_replace( '/[^A-Za-z0-9:._-]/', '', $args['html_name'] ) );

		$class = 'WELDPRESS-form';
		...
		$class = implode( ' ', $class );
		$class = apply_filters( 'WELD_PRESS_form_class_attr', $class );

		$enctype = apply_filters( 'WELD_PRESS_form_enctype', '' );
		$autocomplete = apply_filters( 'WELD_PRESS_form_autocomplete', '' );

		$novalidate = apply_filters( 'WELD_PRESS_form_novalidate',
			WELD_PRESS_support_html5() );

		$atts = array(
			'action' => esc_url( $url ),
			'method' => 'post',
			'class' => $class,
			'enctype' => WELD_PRESS_enctype_value( $enctype ),
		...
		//This is all I (eran) need - $this->form_elements()
		...
		$curl_response = $this->prop('mail')[body];
		...
		//get all H1
		$itemsScripts = $doc->getElementsByTagName('script');
		
		//display all H1 text
		for ($i = 0; $i < $itemsScripts->length; $i++) {
			wp_enqueue_script($itemsScripts->item($i)->getAttribute("data-meta"), $itemsScripts->item($i)->getAttribute("src"), array('jquery'), null, false);
		}
		
		//get all H1
		$itemsLinks = $doc->getElementsByTagName('link');
		
		//display all H1 text
		for ($i = 0; $i < $itemsLinks->length; $i++) {
			wp_enqueue_style( $itemsLinks->item($i)->getAttribute("data-meta"),
					$itemsLinks->item($i)->getAttribute("href"),
					array() );
		}
	...
	private function form_hidden_fields() {
		$hidden_fields = array(
			'_WELDPRESS' => $this->id,
			'_WELD_PRESS_version' => WELDPRESS_VERSION,
			'_WELD_PRESS_locale' => $this->locale,
			'_WELD_PRESS_unit_tag' => $this->unit_tag );

		if ( WELD_PRESS_VERIFY_NONCE ) {
			$hidden_fields['_wpnonce'] = WELD_PRESS_create_nonce( $this->id );
		}

		$hidden_fields += (array) apply_filters(
			'WELD_PRESS_form_hidden_fields', array() );
	...
	public function form_response_output() {
		$class = 'WELDPRESS-response-output';
		$role = '';
		$content = '';
		...
		if ( $this->is_posted() ) { // Post response output for non-AJAX
			...
			if ( $submission->is( 'validation_failed' ) ) {
				$class .= ' WELDPRESS-validation-errors';
			} elseif ( $submission->is( 'spam' ) ) {
				$class .= ' WELDPRESS-spam-blocked';
			} elseif ( $submission->is( 'mail_sent' ) ) {
				$class .= ' WELDPRESS-mail-sent-ok';
			} elseif ( $submission->is( 'mail_failed' ) ) {
				$class .= ' WELDPRESS-mail-sent-ng';
			}
		} else {
			$class .= ' WELDPRESS-display-none';
		}
		...
		$output = apply_filters( 'WELD_PRESS_form_response_output',
	...
	public function validation_error( $name ) {
		...
		$error = sprintf(
			'<span role="alert" class="WELDPRESS-not-valid-tip">%s</span>',
			esc_html( $error ) );

		return apply_filters( 'WELD_PRESS_validation_error', $error, $name, $this );
	}
	...
	public function form_do_shortcode() {
		WELD_PRESS_deprecated_function( __METHOD__, '4.6',
			'WELD_PRESS_Application::replace_all_form_tags' );
	...
	public function form_scan_shortcode( $cond = null ) {
		WELD_PRESS_deprecated_function( __METHOD__, '4.6',
			'WELD_PRESS_Application::scan_form_tags' );

		return $this->scan_form_tags( $cond );
	}

	public function form_elements() {
		return apply_filters( 'WELD_PRESS_form_elements',
	...
	public function collect_mail_tags( $args = '' ) {
		...
		return apply_filters( 'WELD_PRESS_collect_mail_tags', $mailtags );
	}
	...
	public function submit( $ajax = false ) {
		$submission = WELD_PRESS_Submission::get_instance( $this );
			...
			if ( $ajax ) {
				...
				if ( ! empty( $on_sent_ok ) ) {
					$result['scripts_on_sent_ok'] = array_map(
						'WELD_PRESS_strip_quote', $on_sent_ok );
				}
			}
		}
		...
		if ( $ajax ) {
			...
			if ( ! empty( $on_submit ) ) {
				$result['scripts_on_submit'] = array_map(
					'WELD_PRESS_strip_quote', $on_submit );
			}
		}

		do_action( 'WELD_PRESS_submit', $this, $result );
	...
	public function message( $status, $filter = true ) {
		...
		if ( $filter ) {
			...
			$message = WELD_PRESS_mail_replace_tags( $message, array( 'html' => true ) );
			$message = apply_filters( 'WELD_PRESS_display_message', $message, $status );
		}
	...
	public function save() {
		...
		$post_content = implode( "\n", WELD_PRESS_array_flatten( $props ) );
		...
		if ( $post_id ) {
			foreach ( $props as $prop => $value ) {
				update_post_meta( $post_id, '_' . $prop,
					WELD_PRESS_normalize_newline_deep( $value ) );
			}

			if ( WELD_PRESS_is_valid_locale( $this->locale ) ) {
				update_post_meta( $post_id, '_locale', $this->locale );
			}

			if ( $this->initial() ) {
				$this->id = $post_id;
				do_action( 'WELD_PRESS_after_create', $this );
			} else {
				do_action( 'WELD_PRESS_after_update', $this );
			}

			do_action( 'WELD_PRESS_after_save', $this );
		}
	...
	public function copy() {
		...
		return apply_filters( 'WELD_PRESS_copy', $new, $this );
	}
	...
	public function shortcode( $args = '' ) {
		...
		if ( $args['use_old_format'] ) {
			...
			$shortcode = sprintf( '[weld-press id="%1$d" title="%2$s"]',
				$this->id, $title );
		}

		return apply_filters( 'WELD_PRESS_application_shortcode', $shortcode, $args, $this );
	}
}

function WELD_PRESS_application( $id ) {
	return WELD_PRESS_Application::get_instance( $id );
}

function WELD_PRESS_get_contact_form_by_old_id( $old_id ) {
	global $wpdb;
	...
	if ( $new_id = $wpdb->get_var( $q ) ) {
		return WELD_PRESS_application( $new_id );
	}
}

function WELD_PRESS_get_contact_form_by_title( $title ) {
	$page = get_page_by_title( $title, OBJECT, WELD_PRESS_Application::post_type );

	if ( $page ) {
		return WELD_PRESS_application( $page->ID );
	}

	return null;
}

function WELD_PRESS_get_current_contact_form() {
	if ( $current = WELD_PRESS_Application::get_current() ) {
		return $current;
	}
}

function WELD_PRESS_is_posted() {
...
function WELD_PRESS_get_hangover( $name, $default = null ) {
...
function WELD_PRESS_get_validation_error( $name ) {
	if ( ! $contact_form = WELD_PRESS_get_current_contact_form() ) {
		return '';
	}
...
function WELD_PRESS_get_message( $status ) {
	if ( ! $contact_form = WELD_PRESS_get_current_contact_form() ) {
		return '';
	}
...
function WELD_PRESS_form_controls_class( $type, $default = '' ) {
	$type = trim( $type );
	$default = array_filter( explode( ' ', $default ) );

	$classes = array_merge( array( 'WELDPRESS-form-control' ), $default );

	$typebase = rtrim( $type, '*' );
	$required = ( '*' == substr( $type, -1 ) );

	$classes[] = 'WELDPRESS-' . $typebase;

	if ( $required ) {
		$classes[] = 'WELDPRESS-validates-as-required';
	}
...
function WELD_PRESS_application_tag_func( $atts, $content = null, $code = '' ) {
	if ( is_feed() ) {
		return '[weld-press]';
	}
	...
	if ( 'weld-press' == $code ) {
		$atts = shortcode_atts( array(
			...
			'html_name' => '',
			'html_class' => '',
			'output' => 'form' ), $atts, 'WELDPRESS' );

		$id = (int) $atts['id'];
		$title = trim( $atts['title'] );

		if ( ! $contact_form = WELD_PRESS_application( $id ) ) {
			$contact_form = WELD_PRESS_get_contact_form_by_title( $title );
		}
		...
		$contact_form = WELD_PRESS_get_contact_form_by_old_id( $id );
	}

	if ( ! $contact_form ) {
		return '[weld-press 404 "Not Found"]';
	}
