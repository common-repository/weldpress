...
class WELD_PRESS_Submission {
	...
	public static function get_instance( WELD_PRESS_Application $contact_form = null ) {
	...
	private function setup_posted_data() {
		...
		foreach ( (array) $tags as $tag ) {
			...
			&& $pipes instanceof WELD_PRESS_Pipes
			&& ! $pipes->zero() ) {
				if ( is_array( $value) ) {
					$new_value = array();
		...
		$this->posted_data = apply_filters( 'WELD_PRESS_posted_data', $posted_data );
	...
	private function submit() {
		...
		$this->meta = array(
			...
			'url' => preg_replace( '%(?<!:|/)/.*$%', '',
				untrailingslashit( home_url() ) ) . WELD_PRESS_get_request_uri(),
			'timestamp' => current_time( 'timestamp' ),
			'unit_tag' => isset( $_POST['_WELD_PRESS_unit_tag'] )
				? $_POST['_WELD_PRESS_unit_tag'] : '' );
		...
		if ( ! $this->validate() ) { // Validation error occured
			...
			do_action( 'WELD_PRESS_mail_sent', $contact_form );

		} else {
			$this->status = 'mail_failed';
			$this->response = $contact_form->message( 'mail_sent_ng' );

			do_action( 'WELD_PRESS_mail_failed', $contact_form );
		}
	...
	private function validate() {
		...
		require_once WELD_PRESS_PLUGIN_DIR . '/includes/validation.php';
		$result = new WELD_PRESS_Validation();

		$tags = $this->contact_form->scan_form_tags();

		foreach ( $tags as $tag ) {
			$result = apply_filters( 'WELD_PRESS_validate_' . $tag['type'],
				$result, $tag );
		}

		$result = apply_filters( 'WELD_PRESS_validate', $result, $tags );
	...
	private function accepted() {
		return apply_filters( 'WELD_PRESS_acceptance', true );
	}
	...
	private function spam() {
		...
		if ( WELD_PRESS_VERIFY_NONCE && ! $this->verify_nonce() ) {
			$spam = true;
		}
		...
		return apply_filters( 'WELD_PRESS_spam', $spam );
	}

	private function verify_nonce() {
		return WELD_PRESS_verify_nonce( $_POST['_wpnonce'], $this->contact_form->id() );
	}

	private function blacklist_check() {
		$target = WELD_PRESS_array_flatten( $this->posted_data );
		$target[] = $this->get_meta( 'remote_ip' );
		$target[] = $this->get_meta( 'user_agent' );

		$target = implode( "\n", $target );

		return WELD_PRESS_blacklist_check( $target );
	}
	...
	private function mail() {
		...
		do_action( 'WELD_PRESS_before_send_mail', $contact_form );

		$skip_mail = $this->skip_mail || ! empty( $contact_form->skip_mail );
		$skip_mail = apply_filters( 'WELD_PRESS_skip_mail', $skip_mail, $contact_form );

		if ( $skip_mail ) {
			return true;
		}

		$result = WELD_PRESS_Mail::send( $contact_form->prop( 'mail' ), 'mail' );
		...
		if ( $result ) {
			...
			$additional_mail = apply_filters( 'WELD_PRESS_additional_mail',
				$additional_mail, $contact_form );

			foreach ( $additional_mail as $name => $template ) {
				WELD_PRESS_Mail::send( $template, $name );
			}
