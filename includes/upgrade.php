...
add_action( 'WELD_PRESS_upgrade', 'WELD_PRESS_convert_to_cpt', 10, 2 );

function WELD_PRESS_convert_to_cpt( $new_ver, $old_ver ) {
	global $wpdb;
	...
	$table_name = $wpdb->prefix . "weld_press_solution";

	if ( $wpdb->get_var( "SHOW TABLES LIKE '$table_name'" ) ) {
		$old_rows = $wpdb->get_results( "SELECT * FROM $table_name" );
	} elseif ( ( $opt = get_option( 'WELDPRESS' ) ) && ! empty( $opt['contact_forms'] ) ) {
		...
		if ( $post_id ) {
			...
			foreach ( $metas as $meta ) {
				update_post_meta( $post_id, '_' . $meta,
					WELD_PRESS_normalize_newline_deep( maybe_unserialize( $row->{$meta} ) ) );
			}
		}
	}
}

add_action( 'WELD_PRESS_upgrade', 'WELD_PRESS_prepend_underscore', 10, 2 );

function WELD_PRESS_prepend_underscore( $new_ver, $old_ver ) {
	if ( version_compare( $old_ver, '3.0-dev', '<' ) ) {
		return;
	}
	...
	$posts = WELD_PRESS_Application::find( array(
		'post_status' => 'any',
		'posts_per_page' => -1 ) );
